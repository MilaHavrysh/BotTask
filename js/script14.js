...
const isLoginValid = (login, min = 4, max = 16) => {
  const loginValid = min <= login.length && login.length <= max;
  return loginValid;
};
...
//console.log(isLoginValid('rtyhhhhhyyyyyyyyyyyyyyyyyyyyyyyyyy'));
...
//каждая функция делает что - то одно.Это позволяет переиспользовать код и
